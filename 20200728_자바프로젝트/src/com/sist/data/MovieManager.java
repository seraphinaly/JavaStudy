...
public class MovieManager{
	...
	static{
		try{
			...
			//<-여기안에 들어가는건 inputstream이 들어가야한다.
			while(true){
				...
				while(st.hasMoreTokens()){
					...
					vo.setPoster("https:"+st.nextToken()); //영화포스터
					//평점
					String score=st.nextToken();
					score=score.substring(score.indexOf("점")+1); // '평점' 이후에 있는 점수를 가져오기 위해 
					vo.setScore(score.trim()); //공백제거 trim()

					vo.setDirector(st.nextToken()); //감독
					vo.setActor(st.nextToken()); //배우
					vo.setGenre(st.nextToken()); //장르

					String regdate=st.nextToken();
					regdate=regdate.substring(0, regdate.lastIndexOf("개")); //개봉이라는 단어는 필요가 없어서   단어를 자른다.
					vo.setRegdate(regdate);

					String grade=st.nextToken();
					grade=grade.substring(grade.indexOf(",")+1); // ex) 113분, 청소년관람불가 ->에서 ,기준으로 짤라서 '청소년관람불가'는 등급이 된다.
					vo.setGrade(grade.trim());

					//90,000 ==> 90000
					vo.setShowUser(Integer.parseInt(st.nextToken().replace(",", ""))); //관객수
	...
	public ArrayList<MovieVO> movieListData(int page){
		...
		for(MovieVO vo : list){
			//pagecnt => 저장하는 위치 지정
			if(i<10 && j>=pagecnt) //10개가 되면 빠져 나가고 j는 for 돌아가는 횟수 지정 
			{
				movies.add(vo);
				i++;
			}
